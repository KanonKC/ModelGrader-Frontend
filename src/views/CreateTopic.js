...
import { addTopicCollection, createTopic } from "../services/topic.service";
		...
		getAllCollections().then((response) => {
			setcollectionOptions(
				response.data.collections
					.filter((collection) => collection.owner === account_id)
					.map((collection) => ({
				...
				<Row>
					...
					<Col>
						<FormGroup>
							...
							/>
							<img className="mt-2" src={banner} alt="Banner" />
						</FormGroup>
					</Col>
				</Row>
				<Button
